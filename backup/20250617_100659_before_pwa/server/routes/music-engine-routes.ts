...
import { musicStyles } from '../../shared/schema.js';
...
const generateMusicSchema = z.object({
  ...
  preferredEngine: z.enum(['topmedia', 'suno']).optional()
});
...
router.get('/status/:musicId', requireAuth, async (req, res) => {
  try {
    ...
    const result = await musicEngineService.checkStatus(musicId);
...
router.get('/engines', requireAuth, async (req, res) => {
  try {
    ...
    const engineStatus = await musicEngineService.getEngineStatus();
...
router.get('/list', requireAuth, async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      return res.status(500).json({
        success: false,
        error: result.error || '음악 목록 조회에 실패했습니다'
      });
    }
