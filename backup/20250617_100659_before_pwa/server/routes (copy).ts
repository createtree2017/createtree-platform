...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/music/stream/:id", async (req, res) => {
    ...
    try {
      ...
      if (!musicUrl) return res.status(404).send('Music URL not found');
      ...
      // ✅ GCS URL 또는 외부 URL인 경우 (Signed URL 또는 Public URL)
      try {
        ...
        if (!gcsRes.ok || !gcsRes.body) {
          console.error(`GCS fetch failed: ${gcsRes.status} ${gcsRes.statusText}`);
          return res.status(502).send('Invalid audio file or fetch failed');
        }

        // Suno URL의 경우 audio 체크를 완화
        if (!contentType.includes('audio') && !contentType.includes('mpeg') && !musicUrl.includes('suno')) {
          console.error(`Invalid content type: ${contentType}`);
          return res.status(502).send('Invalid audio file');
        }

        // 응답 헤더 복사 (Content-Type 강제 설정)
        res.setHeader('Content-Type', 'audio/mpeg');
        ...
        for (const [key, value] of gcsRes.headers.entries()) {
          if (!['content-type', 'accept-ranges', 'access-control-allow-origin'].includes(key.toLowerCase())) {
        ...
        // ReadableStream을 Node.js Response로 파이핑
