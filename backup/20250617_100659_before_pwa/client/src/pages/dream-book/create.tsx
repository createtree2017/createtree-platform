...
export default function CreateDreamBook() {
  ...
  useEffect(() => {
    if (styles && styles.length > 0 && !form.getValues('styleId')) {
      ...
      if (firstStyle) {
        form.setValue('styleId', firstStyle.styleId || firstStyle.id.toString());
      }
    }
  }, [styles, form]);
  ...
  const addScene = () => {
    if (numberOfScenes < 4) {
      ...
      setActiveScene(numberOfScenes); // 새로 추가된 장면으로 이동
    }
  };

  // 장면 제거 함수
  const removeScene = (sceneIndex: number) => {
    if (numberOfScenes > 1 && sceneIndex > 0) { // 장면 1은 제거할 수 없음
      // 해당 장면의 프롬프트 제거
  ...
  const onSubmit = async (values: z.infer<typeof dreamBookSchema>) => {
    ...
    // 장면 프롬프트 확인 - 현재 선택된 장면 개수만큼만 검증
    const currentScenePrompts = values.scenePrompts.slice(0, numberOfScenes);
    const validScenePrompts = currentScenePrompts.filter(p => p.trim().length > 0);
                    ...
                    >
                      ...
                      <SelectContent>
                        {styles?.map((style: any) => (
                          <SelectItem key={style.id} value={style.styleId || style.id.toString()}>
                            {style.name}
                          </SelectItem>
                        ))}
                      </SelectContent>
              ...
              <CardContent className="p-6">
                ...
                >
                  <TabsList className="w-full justify-start mb-4">
                    {Array.from({ length: numberOfScenes }, (_, index) => (
                    ...
                    {numberOfScenes < 4 && (
                      <button
                        type="button"
                        onClick={addScene}
                        className="flex items-center justify-center px-3 py-2 text-sm font-medium text-gray-500 hover:text-gray-700 border border-dashed border-gray-300 rounded-md hover:border-gray-400"
                      >
                        + 장면 추가
                      </button>
                    )}
