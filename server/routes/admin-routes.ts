...
export function registerAdminRoutes(app: Express): void {
  ...
  // 1. 발송 내역 모니터링
  app.get("/api/admin/push-logs", requireAdminOrSuperAdmin, async (req: Request, res: Response) => {
    try {
      const page = parseInt(req.query.page as string) || 1;
      const limit = parseInt(req.query.limit as string) || 50;
      const offset = (page - 1) * limit;

      const { pushDeliveryLogs } = await import("../../shared/schema");
  ...
  // 2. 수동 알림 발송 (테스트 및 전체/특정 대상 발송)
  const pushSendLimiter = rateLimit({
    windowMs: 60 * 1000, // 1분
    max: 5, // 분당 최대 5회 (개별/병원 발송 기준)
    message: { error: "푸시 발송 요청이 너무 많습니다. 1분 후 다시 시도해주세요." },
    standardHeaders: true,
    legacyHeaders: false,
  });

  app.post("/api/admin/push-send", requireAdminOrSuperAdmin, pushSendLimiter, async (req: Request, res: Response) => {
    try {
      const { targetType, targetIds, title, body, actionUrl, imageUrl } = req.body;
