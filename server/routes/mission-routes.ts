...
async function buildMissionTree(
  missionId: number,
  userId: number,
  depth: number = 1,
...
// 카테고리 수정
router.put(
  "/admin/mission-categories/:id",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const categoryData = missionCategoriesInsertSchema
        .partial()
        .parse(req.body);
...
// 카테고리 순서 변경
router.patch(
  "/admin/mission-categories/reorder",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      if (!file) {
        return res
          .status(400)
          .json({ success: false, error: "이미지 파일이 필요합니다" });
      }
...
router.get("/admin/missions", requireAdminOrSuperAdmin, async (req, res) => {
  try {
    ...
    if (visibilityType) {
      conditions.push(
        eq(themeMissions.visibilityType, visibilityType as string),
      );
    }

    if (hospitalId) {
      conditions.push(
        eq(themeMissions.hospitalId, parseInt(hospitalId as string)),
      );
    }
    ...
    if (parentMissionId) {
      conditions.push(
        eq(themeMissions.parentMissionId, parseInt(parentMissionId as string)),
      );
    }
...
router.post("/admin/missions", requireAdminOrSuperAdmin, async (req, res) => {
  try {
    ...
    if (
      error.code === "23505" &&
      error.constraint === "theme_missions_mission_id_key"
    ) {
      return res
        .status(400)
        .json({
          error: "이미 존재하는 미션 ID입니다. 다른 ID를 사용해주세요.",
        });
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `[미션 복사] 새 미션 생성됨:`,
        JSON.stringify(newMission, null, 2),
      );
      ...
      console.log(
        `[미션 복사] "${original.title}" → "[복사본] ${original.title}" (ID: ${newMission.id})`,
      );
      res.status(201).json(newMission);
    } catch (error: any) {
      ...
      if (error.code === "23505") {
        return res
          .status(400)
          .json({
            error: "복사 중 중복 오류가 발생했습니다. 다시 시도해주세요.",
          });
      }
...
router.get(
  "/admin/missions/:parentId/child-missions",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
...
router.get(
  "/admin/missions/:parentId/approved-users",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      const parentId = parseInt(req.params.parentId);
...
// 주제 미션 활성화/비활성화 토글
router.patch(
  "/admin/missions/:id/toggle-active",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      const id = parseInt(req.params.id);
...
// 세부 미션 목록 조회
router.get(
  "/admin/missions/:missionId/sub-missions",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      const { missionId } = req.params;
  ...
  async (req, res) => {
    try {
      ...
      if (requestData.sequentialLevel !== undefined) {
        requestData.sequentialLevel =
          parseInt(requestData.sequentialLevel) || 0;
      } else {
...
// 세부 미션 수정
router.put(
  "/admin/missions/:missionId/sub-missions/:subId",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      ...
      const subMissionData = subMissionsInsertSchema
        .partial()
        .parse(requestData);
...
router.patch(
  "/admin/missions/:missionId/sub-missions/reorder",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      ...
      if (!Array.isArray(subMissionIds)) {
        return res
          .status(400)
          .json({ error: "subMissionIds는 배열이어야 합니다" });
      }
...
// 세부 미션 활성화/비활성화 토글
router.patch(
  "/admin/missions/:missionId/sub-missions/:subId/toggle-active",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      const subId = parseInt(req.params.subId);
  ...
  try {
    ...
    const missionsWithProgress = await Promise.all(
      missions.map(async (mission) => {
        ...
        // 디버그 로그
        console.log(
          `[미션 ${mission.id}] 세부미션 수: ${mission.subMissions.length}, 신청미션: ${applicationSubMission ? "있음" : "없음"}`,
        );
        if (applicationSubMission) {
          console.log(
            `[미션 ${mission.id}] 신청미션 날짜: ${applicationSubMission.startDate} ~ ${applicationSubMission.endDate}`,
          );
        }
...
router.get(
  "/missions/:parentId/child-missions",
  requireAuth,
  async (req, res) => {
    try {
      const parentId = parseInt(req.params.parentId);
...
router.get("/missions/:missionId", requireAuth, async (req, res) => {
  try {
    ...
    if (mission.parentMissionId) {
      ...
      const validateAncestorChain = async (
        missionId: number,
      ): Promise<{ valid: boolean; blockerMission?: any }> => {
      ...
      if (
        !parentProgress ||
        parentProgress.status !== MISSION_STATUS.APPROVED
      ) {
        const parentMission = await db.query.themeMissions.findFirst({
          where: eq(themeMissions.id, mission.parentMissionId),
        });
      ...
      if (parentMission?.parentMissionId) {
        const ancestorResult = await validateAncestorChain(
          parentMission.parentMissionId,
        );
      ...
      if (parentMissionData?.parentMissionId) {
        ...
        const parentSiblings = await db.query.themeMissions.findMany({
          where: and(
            eq(
              themeMissions.parentMissionId,
              parentMissionData.parentMissionId,
            ),
            eq(themeMissions.isActive, true),
          ),
        });
          ...
          if (originalData) {
            ...
            if (data.slots && Array.isArray(data.slots)) {
              data.slots = data.slots.map((slot: any) => ({
                ...slot,
                fileUrl:
                  slot.fileUrl && slot.gsPath
                    ? ensurePermanentUrl(slot.fileUrl, slot.gsPath)
                    : slot.fileUrl,
                imageUrl:
                  slot.imageUrl && slot.gsPath
                    ? ensurePermanentUrl(slot.imageUrl, slot.gsPath)
                    : slot.imageUrl,
              }));
            }
    ...
    const isCurrentMissionApproved =
      progress?.status === MISSION_STATUS.APPROVED;
      ...
      childMissions.map(async (childMission, index) => {
        ...
        const childProgressPercentage =
          childTotalSubMissions > 0
            ? Math.round(
                (childCompletedSubMissions / childTotalSubMissions) * 100,
              )
            : 0;
        ...
        if (isUnlocked && mission.parentMissionId) {
          ...
          for (const sibling of parentSiblings) {
            const siblingProgress =
              await db.query.userMissionProgress.findFirst({
                where: and(
                  eq(userMissionProgress.userId, userId),
                  eq(userMissionProgress.themeMissionId, sibling.id),
                  eq(userMissionProgress.status, MISSION_STATUS.APPROVED),
    ...
    if (applicationSubMission) {
      if (mission.isFirstCome) {
        ...
        const approvedApplications = await db
          .select({
            count: sql<number>`count(DISTINCT ${subMissionSubmissions.userId})::int`,
          })
          .from(subMissionSubmissions)
          .where(
            and(
...
router.post("/missions/:missionId/start", requireAuth, async (req, res) => {
  try {
    ...
    if (existingProgress) {
      return res
        .status(400)
        .json({ error: "이미 시작한 미션입니다", progress: existingProgress });
    }
...
// 세부 미션 제출
router.post(
  "/missions/:missionId/sub-missions/:subMissionId/submit",
  requireAuth,
  async (req, res) => {
    try {
...
// 세부 미션 제출 취소
router.delete(
  "/missions/:missionId/sub-missions/:subMissionId/submission",
  requireAuth,
  async (req, res) => {
    try {
      const { missionId, subMissionId } = req.params;
      ...
      if (submission.isLocked) {
        return res
          .status(403)
          .json({ error: "승인된 세부 미션은 취소할 수 없습니다" });
      }
...
router.post(
  "/missions/:missionId/sub-missions/:subMissionId/cancel-application",
  requireAuth,
  async (req, res) => {
    try {
      const { missionId, subMissionId } = req.params;
      ...
      if (subMission.actionType?.name !== "신청") {
        return res
          .status(400)
          .json({ error: "신청 타입 세부미션만 취소할 수 있습니다" });
      }
      ...
      if (
        subMission.themeMission?.isFirstCome &&
        previousStatus === MISSION_STATUS.APPROVED
      ) {
        // 대기 중인 사람 중 가장 먼저 제출한 사람 찾기
        const waitlistSubmission =
          await db.query.subMissionSubmissions.findFirst({
            where: and(
              eq(subMissionSubmissions.subMissionId, parseInt(subMissionId)),
              eq(subMissionSubmissions.status, MISSION_STATUS.WAITLIST),
            ),
        ...
        if (waitlistSubmission) {
          ...
          console.log(
            `✅ [신청 취소] 대기자 ${waitlistSubmission.userId} 자동 승인`,
          );
        }
      }
...
router.post("/missions/:missionId/complete", requireAuth, async (req, res) => {
  try {
    ...
    if (!progress) {
      return res
        .status(404)
        .json({ error: "미션 진행 내역을 찾을 수 없습니다" });
    }
    ...
    try {
      ...
      if (userRole === "hospital_admin" && hospitalId) {
        return res
          .status(403)
          .json({
            error: "병원 관리자는 다른 병원의 데이터를 조회할 수 없습니다",
          });
      }
...
router.get(
  "/admin/review/theme-missions/:missionId/sub-missions",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      const { missionId } = req.params;
      ...
      if (userRole === "hospital_admin") {
        ...
        if (
          mission.visibilityType === VISIBILITY_TYPE.HOSPITAL &&
          mission.hospitalId !== userHospitalId
        ) {
          return res.status(403).json({ error: "접근 권한이 없습니다" });
        }
      }
  ...
  async (req, res) => {
    try {
      ...
      if (userRole === "hospital_admin") {
        ...
        const accessibleSubMissionIds = accessibleSubMissions.map(
          (sm) => sm.id,
        );
        ...
        if (subMissionId) {
          ...
          conditions.push(
            eq(subMissionSubmissions.subMissionId, requestedSubMissionId),
          );
        }
      ...
      const processedSubmissions = submissions.map((submission: any) => {
        ...
        if (processedData.fileUrl && processedData.gsPath) {
          processedData.fileUrl = ensurePermanentUrl(
            processedData.fileUrl,
            processedData.gsPath,
          );
        }
        if (processedData.imageUrl && processedData.gsPath) {
          processedData.imageUrl = ensurePermanentUrl(
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // 병원 관리자는 자기 병원만 조회
      let filterHospitalId = hospitalId
      ...
      const filteredSubmissions = filterHospitalId
        ? pendingSubmissions.filter(
            (s) => s.subMission.themeMission.hospitalId === filterHospitalId,
          )
        : pendingSubmissions;
...
// 세부 미션 승인
router.post(
  "/admin/review/submissions/:submissionId/approve",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
...
// 세부 미션 거절
router.post(
  "/admin/review/submissions/:submissionId/reject",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      const submissionId = parseInt(req.params.submissionId);
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "시스템 기본 액션 타입의 이름은 변경할 수 없습니다" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (existing.isSystem) {
        return res
          .status(400)
          .json({ error: "시스템 기본 액션 타입은 삭제할 수 없습니다" });
      }
      ...
      if (usedInMissions) {
        return res
          .status(400)
          .json({ error: "사용 중인 액션 타입은 삭제할 수 없습니다" });
      }
...
// 출석 비밀번호 확인 (사용자용)
router.post(
  "/sub-missions/:id/verify-attendance",
  requireAuth,
  async (req, res) => {
    try {
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      // 폴더 삭제
      await db.delete(missionFolders).where(eq(missionFolders.id, folderId));
...
router.get(
  "/admin/missions/:missionId/export-excel",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      // Zod 파라미터 검증
      ...
      for (const subMission of subMissionList) {
        ...
        const extractSlotRows = (
          ...
          if (data.slots && Array.isArray(data.slots)) {
            data.slots.forEach((slot: any, index: number) => {
              ...
              if (slot.linkUrl)
                rows.push({
                  format: `${slotLabel}[링크]`,
                  content: slot.linkUrl,
                });
              if (slot.imageUrl)
                rows.push({
                  format: `${slotLabel}[이미지]`,
                  content: slot.imageUrl,
                });
              if (slot.fileUrl)
                rows.push({
                  format: `${slotLabel}[파일]`,
                  content: slot.fileUrl,
                });
              if (slot.textContent)
                rows.push({
                  format: `${slotLabel}[텍스트]`,
          ...
          if (
            ...
            if (data.linkUrl)
              rows.push({ format: "링크", content: data.linkUrl });
            if (data.imageUrl)
              rows.push({ format: "이미지", content: data.imageUrl });
            if (data.fileUrl)
              rows.push({ format: "파일", content: data.fileUrl });
            if (data.textContent)
              rows.push({ format: "텍스트", content: data.textContent });
          ...
          if (data.registrationPhone)
            rows.push({
              format: "신청연락처",
              content: data.registrationPhone,
            });
...
router.get("/big-missions", requireAuth, async (req, res) => {
  try {
    ...
    const visibilityCondition = isSuperAdmin
      ? or(
          ...
          and(
            ...
            userHospitalId
              ? eq(bigMissions.hospitalId, userHospitalId)
              : sql`false`,
          ),
        )
      : or(
    ...
    const missions = await db.query.bigMissions.findMany({
      where: and(eq(bigMissions.isActive, true), visibilityCondition),
      with: {
        topics: {
    ...
    const missionsWithProgress = await Promise.all(
      missions.map(async (mission) => {
        ...
        // 이 큰미션에 필요한 카테고리의 approved 기록을 한 번에 조회
        const neededCategoryIds = [
          ...new Set(mission.topics.map((t) => t.categoryId)),
        ];
        const allApprovedForList = await db
          .select({
            categoryId: themeMissions.categoryId,
            approvedAt: userMissionProgress.updatedAt,
          })
          .from(userMissionProgress)
          .innerJoin(
            themeMissions,
            eq(userMissionProgress.themeMissionId, themeMissions.id),
          )
          .where(
            and(
              eq(userMissionProgress.userId, userId),
              eq(userMissionProgress.status, MISSION_STATUS.APPROVED),
              inArray(themeMissions.categoryId, neededCategoryIds),
            ),
          )
          .orderBy(asc(userMissionProgress.updatedAt));

        // 카테고리별 사용 가능한 approved 횟수 맵 생성
        const approvedCountMap = new Map<string, number>();
        for (const a of allApprovedForList) {
          if (!a.categoryId) continue;
          approvedCountMap.set(
            a.categoryId,
            (approvedCountMap.get(a.categoryId) ?? 0) + 1,
          );
        }

        // 토픽 순서대로 approved 기록 1:1 소비하여 완료 수 계산
        const usedCountMap = new Map<string, number>();
        let completedTopics = 0;
        for (const topic of mission.topics) {
          const available = approvedCountMap.get(topic.categoryId) ?? 0;
          const used = usedCountMap.get(topic.categoryId) ?? 0;
          if (available > used) {
            completedTopics++;
            usedCountMap.set(topic.categoryId, used + 1);
          }
        }
...
router.get("/big-missions/:id", requireAuth, async (req, res) => {
  try {
    ...
    // 각 토픽의 완료 여부 계산 (같은 카테고리 N개 토픽 → N번 완료 필요)
    // 1. 이 큰미션에 필요한 카테고리의 approved 기록을 한 번에 조회
    const neededCategoryIds = [
      ...new Set(mission.topics.map((t) => t.categoryId)),
    ];
    const allApproved = await db
      ...
      .orderBy(asc(userMissionProgress.updatedAt)); // 오래된 것부터 소비

    // 2. 카테고리별 사용 가능한 approved 기록 풀 생성
    const approvedPool = new Map<string, typeof allApproved>();
    for (const m of allApproved) {
      if (!m.categoryId) continue;
      if (!approvedPool.has(m.categoryId)) approvedPool.set(m.categoryId, []);
      approvedPool.get(m.categoryId)!.push(m);
    }

    // 3. 토픽 순서대로 approved 기록을 1개씩 소비하며 완료 여부 결정
    const topicsWithCompletion = mission.topics.map((topic) => {
      const pool = approvedPool.get(topic.categoryId) ?? [];
      const consumed = pool.shift(); // 가장 오래된 기록부터 1개 소비
      const isCompleted = !!consumed;
      return {
        ...topic,
        isCompleted,
        completedMissionTitle: consumed?.title ?? null,
      };
    });

    const totalTopics = mission.topics.length;
    const completedTopics = topicsWithCompletion.filter(
      (t) => t.isCompleted,
    ).length;
    const progressPercent =
...
// 관리자: 큰미션 활성/비활성 토글
router.patch(
  "/admin/big-missions/:id/toggle-active",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
      const id = parseInt(req.params.id);
...
// 관리자: 토픽 목록 조회
router.get(
  "/admin/big-missions/:bigMissionId/topics",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
...
// 관리자: 토픽 수정
router.put(
  "/admin/big-missions/:bigMissionId/topics/:topicId",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
...
// 관리자: 토픽 순서 변경 (일괄)
router.put(
  "/admin/big-missions/:bigMissionId/topics/reorder",
  requireAdminOrSuperAdmin,
  async (req, res) => {
    try {
