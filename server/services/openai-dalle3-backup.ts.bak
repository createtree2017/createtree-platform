...
주의: AI가 이미지를 변환할 때 원본 인물의 모든 특징을 정확히 유지하는 것이 매우 중요합니다. 최대한 상세하고 명확하게 작성해 주세요.`
        },
        {
          type: "image_url",
          image_url: {
    ...
    const promptGenerationBody = {
      ...
      messages: [
        {
          role: "system",
          content: `You are an image prompt engineer specialized in applying dramatic style transformations while preserving facial identity. Your task is to generate a prompt that allows the image model (gpt-image-1) to completely transform the scene according to user requirements, while maintaining the person's facial features.

Important Guidelines:

1. **Facial identity preservation** (CRITICAL):
   - Preserve exact eye shape, size, nose structure, mouth shape, and facial proportions
   - Maintain the person's unique facial characteristics and expression
   - Keep hair color and general hairstyle recognizable

2. **Allow dramatic scene transformation** (IMPORTANT):
   - **COMPLETELY CHANGE backgrounds** according to user requirements (stages, animation worlds, fantasy settings)
   - **TRANSFORM clothing and accessories** to match the requested style
   - **APPLY lighting, effects, and atmosphere** as specified in user prompt
   - **CHANGE camera angles and composition** as requested

3. **Style application priority**:
   - User-requested backgrounds take precedence over original setting
   - Apply requested visual effects (3D animation, stage lighting, neon signs, etc.)
   - Transform overall scene composition to match user vision

4. **Balance preservation and transformation**:
   - Keep the person recognizable but allow their environment to be completely reimagined
   - Preserve facial features while allowing dramatic style changes to everything else

The final prompt must:
- **Prioritize user's creative vision** for backgrounds and settings
- Allow complete scene transformation while keeping the person identifiable
- Include all requested visual elements (stages, lighting, 3D effects, etc.)
- Focus on bringing user's vision to life, not just preserving original elements

Use phrases like:
- "maintaining the person's distinctive facial features while"
- "transforming the entire scene to show"
- "placing the recognizable person in"
- "completely changing the background to"`
        },
        {
          role: "user",
...
${prompt ? `위 정보를 바탕으로 인물의 얼굴 특성(눈, 코, 입, 표정)만 유지하면서 사용자가 요청한 새로운 스타일과 배경을 완전히 적용하는 프롬프트를 작성해 주세요. 

중요한 지침:
1. 인물의 얼굴은 인식 가능하도록 유지
2. 사용자가 요청한 배경, 무대, 조명, 효과는 **완전히 새롭게** 적용
3. 원본 배경은 무시하고 사용자 요청 배경으로 **완전 교체**
4. 의상, 소품, 카메라 각도도 사용자 요청에 맞게 변경
5. 3D 애니메이션, 무대 효과 등 시각적 스타일은 사용자 지시를 우선 적용

프롬프트는 사용자의 창작 비전을 최우선으로 하되, 인물만 알아볼 수 있게 작성해주세요.` : 
`사용자가 프롬프트를 지정하지 않았습니다. 인물의 얼굴 특성만 유지하고 나머지는 간단한 스타일 변환을 적용해주세요.`}`
        }
      ],
      max_tokens: 1000
    };
...
${systemInstructions}
Key characteristics to preserve: ${isChild ? "This is a CHILD - DO NOT AGE UP. " : ""}Maintain exact facial features and expression while allowing complete transformation of pose, clothing, and background as requested by user.`;
...
export async function transformImage(
  ...
  try {
    ...
    // 텍스트 전용 생성 vs 이미지 기반 생성 분기 처리
    let imageUrl = "";
    let safetyError = false;
    
    if (imageBuffer === null) {
      // 🆕 텍스트 전용 생성: 이미지 참조 없이 GPT-Image-1 직접 사용
      console.log("텍스트 전용 생성: GPT-Image-1 직접 호출");
      
      while (retryCount <= maxRetries) {
        try {
          // 텍스트 전용이므로 GPT-Image-1을 직접 사용
          imageUrl = await generateImage(promptText);
          
          if (imageUrl.includes("safety_system")) {
            console.log(`안전 시스템 오류 발생 (시도 ${retryCount + 1}/${maxRetries + 1})`);
            safetyError = true;
            promptText = "Create a simple artistic illustration with soft colors and friendly design.";
            retryCount++;
          } else {
            break;
          }
        } catch (retryError) {
          console.error(`텍스트 전용 생성 재시도 중 오류 (시도 ${retryCount + 1}/${maxRetries + 1}):`, retryError);
          retryCount++;
      ...
      if (imageUrl !== SERVICE_UNAVAILABLE && !imageUrl.includes("safety_system")) {
        console.log("텍스트 전용 이미지 생성 성공 (GPT-Image-1)");
      }
    } else {
      // 🔄 기존 이미지 기반 생성: GPT-4o Vision + gpt-image-1
