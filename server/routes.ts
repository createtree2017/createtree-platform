...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 관리자용 마일스톤 API, 배너 API는 admin-routes.ts로 이동됨
  ...
  // 썸네일 이미지 업로드 API, Service Categories/Items API는 admin-routes.ts로 이동됨
  ...
  // ==================== 음악 갤러리 관리 API ====================
  // 음악 관리 API는 admin-routes.ts로 이동됨

  // 통합 음악 엔진 API 라우트 등록
  app.use('/api/music-engine', musicEngineRouter);
  app.use('/api/music', musicEngineRouter); // 기존 /api/music 경로 호환성 유지
  ...
  // 🌐 GCS 이미지 관리 API는 admin-routes.ts로 이동됨
