...
export class DatabaseStorage {
  ...
  async transformImage(filePath: string, style: string, customPromptTemplate?: string | null, systemPrompt?: string | null) {
    try {
      ...
      // OpenAI 서비스 사용
      const { transformWithOpenAI } = await import('./services/openai-dalle3'); 
      const transformedImageUrl = await transformWithOpenAI(
        customPromptTemplate || style, 
        imageBuffer,
        systemPrompt || undefined,
        undefined // variables should be Record<string, string> or undefined
      );
  ...
  // 채팅 관련 메소드들 (비활성화된 기능용 스텁)
  async getChatHistory() {
    console.log('[Storage] 채팅 기록 조회 요청 - 기능이 비활성화됨');
    return [];
  }

  async createShareLink(id: number, type: string) {
    console.log(`[Storage] 공유 링크 생성 요청 - ID: ${id}, 타입: ${type}`);
    // 기본 공유 URL 반환
    return `/shared/${type}/${id}`;
  }

  async saveChat(validatedData: any) {
    console.log('[Storage] 채팅 저장 요청 - 기능이 비활성화됨');
    return {
      error: '채팅 저장 기능이 비활성화되었습니다.',
      message: 'Chat saving functionality is currently disabled.'
    };
  }

  async getSavedChat(chatId: number) {
    console.log(`[Storage] 저장된 채팅 조회 요청 - ID: ${chatId} - 기능이 비활성화됨`);
    return null;
  }

  async deleteSavedChat(chatId: number) {
    console.log(`[Storage] 저장된 채팅 삭제 요청 - ID: ${chatId} - 기능이 비활성화됨`);
    return {
      success: false,
      message: '채팅 삭제 기능이 비활성화되었습니다.'
    };
  }
}
