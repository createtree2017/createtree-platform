...
export default function MilestoneCategoryManagement() {
  ...
  const { data: categories = [], isLoading } = useQuery({
    queryKey: ['/api/admin/milestone-categories'],
    queryFn: async () => {
      try {
        console.log("🔍 관리자 카테고리 API 요청 시작");
        const response = await apiRequest('/api/admin/milestone-categories');
        console.log("🔍 관리자 카테고리 API Response 객체:", response);
        
        // Response 객체에서 JSON 추출
        const jsonData = await response.json();
        console.log("🔍 관리자 카테고리 API JSON 데이터:", jsonData);
        
        // 응답이 배열인지 확인하고, 그렇지 않으면 빈 배열 반환
        const result = Array.isArray(jsonData) ? jsonData : [];
        console.log("🔍 최종 카테고리 데이터:", result);
        return result;
      } catch (error) {
