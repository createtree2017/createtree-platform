...
Backend:
├── Node.js (v20) + Express.js + Railway PostgreSQL (표준 pg 드라이버)
├── Drizzle ORM (node-postgres) + Zod Validation
...
## 4. 인프라스트럭처 및 데이터베이스 운영 전략

### 4.1 Railway PostgreSQL 이관 (v3.3 신규)
- **데이터베이스 이관 완료 (2026.03.22)**: Neon Serverless DB(WebSocket 드라이버) → Railway PostgreSQL(표준 `pg` TCP 드라이버)로 완전 이관.
- **이관 배경**: Neon의 서버리스 Cold Start 문제, WebSocket 연결 불안정성, 월 ~$19 유료 플랜. Railway PG는 동일 인프라 내 Always-On으로 실행되어 지연시간 최소화.
- **코드 변경**: `db/index.ts`에서 `@neondatabase/serverless` → `pg` 표준 Pool, `drizzle-orm/neon-serverless` → `drizzle-orm/node-postgres`로 교체.
- **데이터 무결성**: 63개 테이블, 6,922행 전량 이관 완료 (1건 무효 데이터 제외: `user_settings` user_id='NaN').
- **운영팀 필수 지침**: DB 스크립트 작성 시 `@neondatabase/serverless`가 아닌 표준 `pg` 패키지를 사용해야 함. 스크립트 템플릿은 `GEMINI.md` 참고.
...
**최종 검토/업데이트 일시:** 2026년 3월 22일 (v3.3 Railway PG 이관 반영)
