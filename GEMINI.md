...
> **이 규칙은 GEMINI.md의 모든 다른 규칙보다 우선하며, 어떠한 예외도 없이 항상 적용된다.**
...
- 필수 답변규칙
  ...
  - !!승인!! 이라는 키워드가 포함된 경우, 코드 수정 및 터미널 실행 권한을 포함한 모든 개발 권한을 이관받아 승인 절차 없이 끝까지 개발을 수행합니다. (단, git add / git commit / git push는 제외 — 🔴 최우선 규칙 참고)
  - !!푸시!! 이라는 키워드가 포함된 경우에만 `git add -A` → `git commit` → `git push origin develop` 순서로 실행합니다. 커밋 메시지에는 작업 내용의 핵심을 한국어로 요약합니다.
  - !!테스트!! 이라는 키워드가 포함된 경우에만, 브라우저 에이전트를 직접 실행하여 정밀 테스트를 진행합니다.
