...
- **저장 위치**: `docs/04-report/features/`
- **파일명 형식**: `1-{YYYYMMDD}-{기능요약_한글}.report.md`
  - **시작일** (YYYYMMDD): 최초 작성일, **고정** (변경 불가)
  - **마지막 작업일** (MMDD): 다른 날 수정 시 추가, **매번 최신일로 덮어쓰기**
  - 같은 날 수정 시에는 마지막일 없이 기존 파일 업데이트
  ```
  최초 작성:     1-20260220-나의미션_히스토리_탭_수정.report.md
  다른 날 수정:  1-20260220-0225-나의미션_히스토리_탭_수정.report.md
  또 다른 날:    1-20260220-0301-나의미션_히스토리_탭_수정.report.md  ← 0225→0301 덮어쓰기
  ```
...
5. 문서를 `docs/04-report/features/1-{YYYYMMDD}-{기능요약_한글}.report.md`에 저장합니다.

6. 동시에 `docs/1_Detailed-Development-Specifications/` 규칙에 맞게 상세 스펙 문서도 업데이트를 제안합니다:
   - 파일명 규칙: `1-{YYYYMMDD}-{기능요약_한글}.md`
