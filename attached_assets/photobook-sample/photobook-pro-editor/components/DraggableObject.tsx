...
interface DraggableObjectProps {
  ...
  isPanningMode?: boolean; // New prop for panning state
...
export const DraggableObject: React.FC<DraggableObjectProps> = ({
  ...
  // Helper to project screen delta onto rotated local axes
  ...
  // Handle Dragging (Moving the Frame)
  const handleMouseDown = (e: React.MouseEvent) => {
    if (renderLayer === 'overlay') return; // Only drag from content layer to avoid conflict
    e.stopPropagation();
    onSelect(e);
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      ...
      // Rounding coordinates prevents sub-pixel rendering artifacts
      onUpdate(object.id, {
  ...
  const handleRotateStart = (e: React.MouseEvent) => {
    e.stopPropagation();
    // ElementRef must be attached to the rendered div for this to work
  ...
  return (
    <div
      ...
      style={{
        ...
        width: `${object.width}px`,
        height: `${object.height}px`,
        zIndex: object.zIndex, // Match zIndex so overlays stack correctly
      }}
    >
        {/* Only show controls if selected AND not panning */}
        {isSelected && !isPanningMode && (
            <>
                {/* Outline - Pointer events none so clicks go through to content for dragging */}
                ...
                {/* Crop Handles (Sides) - Pointer events auto */}
                <div 
                ...
                {/* Scale Handles (Corners) - Pointer events auto */}
                <div 
