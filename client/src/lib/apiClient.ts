...
export const api = {
  ...
  getImageList: (page = 1, pageSize = 20, filterByUser = true) => {
    ...
    return getApi('/api/image', params);
  },
  ...
  // 파일 다운로드 (iOS PWA 호환)
  downloadMedia: async (id: number, type: string, title?: string) => {
    try {
      ...
      const filename = `${type}-${id}.${type === 'music' ? 'mp3' : 'jpg'}`;
      const displayTitle = title || `${type}-${id}`;
      
      // iOS PWA 환경에서는 향상된 다운로드 방식 사용
      const { detectPlatform } = await import('../utils/platform-detection');
      const platform = detectPlatform();
      
      if (platform.isIOSPWA && type === 'image') {
        // 이미지의 경우 iOS PWA 전용 다운로드 로직 사용
        const { downloadImageSafely } = await import('../utils/ios-pwa-download');
        
        // 먼저 이미지 URL을 얻기 위해 endpoint 호출
        const imageResponse = await fetch(endpoint, { method: 'HEAD' });
        const imageUrl = imageResponse.url || endpoint;
        
        const result = await downloadImageSafely(imageUrl, endpoint, displayTitle, filename);
      ...
      // 기존 방식 (일반 브라우저 또는 음악 파일)
      const link = document.createElement('a');
      link.href = endpoint;
      link.download = filename;
      link.style.display = 'none';
      
      // 모바일 호환성을 위해 body에 추가
