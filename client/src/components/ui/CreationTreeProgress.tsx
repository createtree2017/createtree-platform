...
export default function CreationTreeProgress({
    ...
    return (
        <div className="w-full relative flex flex-col items-center mb-8 mt-2">
            {/* Status Pill */}
            <div className="bg-black/30 backdrop-blur-md px-5 py-2 rounded-full flex items-center gap-2 border border-white/10 z-10 mb-3">
            ...
            <div className="w-full relative flex justify-center py-4 items-center">
                ...
                <div
                    className="relative w-full max-w-[1024px] flex items-center justify-center"
                    key={`tree-level-${currentLevel}`}
                >
                    <img
                        src={treeImagePath}
                        alt={`Level ${currentLevel} Tree`}
                        className="block w-full h-auto object-contain drop-shadow-2xl transition-all duration-700"
                        onError={(e) => {
